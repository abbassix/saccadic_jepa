...
class ImageNetBaseDataset(Dataset):
    def __init__(self, config):
        ...
        # self.root = Path(config.dataset_path).expanduser()
...
class ImagePatchLocatingDataset(ImageNetBaseDataset):
    ...
    def _get_random_crop_bounds(self, rng=None):
        """
        Sample a crop origin. If `rng` is provided (a random.Random instance),
        use it instead of the global `random` module so callers can get
        deterministic, reproducible crops (e.g. for a fixed val benchmark)
        without disturbing global RNG state used elsewhere (e.g. train
        augmentations, shuffling).
        """
        max_offset = self.img_size - self.crop_size
        source = rng if rng is not None else random
        x = source.randint(0, max_offset)
        y = source.randint(0, max_offset)
        return x, y
    ...
    def __getitem__(self, idx):
        ...
        if self.config.split == "val":
            ...
            x_ref, y_ref = self._get_random_crop_bounds(rng)
            x_goal, y_goal = self._get_random_crop_bounds(rng)
        else:
            x_ref, y_ref = self._get_random_crop_bounds()
            x_goal, y_goal = self._get_random_crop_bounds()
